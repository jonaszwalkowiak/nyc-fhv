...
Projekt przedstawiający cały cykl operacji na danych - od pobrania ich ze źródeł zewnętrznych, przez składowanie w Google Cloud Storage, aż po przetwarzanie w BigQuery. Celem jest praktyczne wykorzystanie kluczowych technologii i wzorców stosowanych w data engineeringu.


1. Pobranie surowych danych z API (NYC Open Data + OpenWeatherMap) przy użyciu `Airflow` DAG i zapis w `GCS`
2. Załadowanie danych do tabeli (***bronze***) `BigQuery` przy użyciu `Airflow` DAG
